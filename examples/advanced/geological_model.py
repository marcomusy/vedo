...
Negro_Mag_Fault_vertices = Points(
    Negro_Mag_Fault_verticesPD.values
).generate_delaunay2d(mode="fit")
Negro_Mag_Fault_vertices.name = "Negro Fault"
plt += Negro_Mag_Fault_vertices.c("f").opacity(0.6)

# Opal fault
Opal_Mound_Fault_vertices = Points(
    Opal_Mound_Fault_verticesPD.values
).generate_delaunay2d(mode="fit")
...
top_granitoid_vertices = (
    Points(xyz).generate_delaunay2d().texture(dataurl + "textures/paper2.jpg")
)
