...
vedo.show(
    "Test of  1/f = (n-1) :dot (1/R1-1/R2) :approx 1/2",
    elements,
    lines,
    lens.boundaries().lw(2),
    azimuth=-90,
    zoom=1.2,
    size=(1100, 700),
    axes=dict(zxgrid=True),
).close()
...
################################################################ spherical mirrors
s1 = (
    vedo.Sphere(r=7, res=50)
    .cut_with_plane([0, 0, 6], "z")
    .cut_with_cylinder(invert=True)
)
...
source = (
    vedo.Disc(r1=1, r2=3, res=[20, 60]).cut_with_plane().cut_with_plane(normal="y").z(1)
)
...
source = (
    vedo.Disc(r1=0.1, r2=0.3, res=[10, 30]).cut_with_plane().cut_with_plane(normal="y")
)
lines = [Ray(pt).trace(elements).asLine() for pt in source.points]
vedo.show(
    "Reflection from a parabolic mirror", elements, lines, axes=2, azimuth=-90
).close()
...
vedo.show(
    ...
    detector,
    lines,
    "A Mesh mirror and a spherical detector",
    elevation=-90,
    axes=1,
    bg="bb",
    bg2="blue9",
