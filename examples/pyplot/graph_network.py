...
for i in range(6):
    g.add_child(i)  # add one child node to node i
for i in range(3):
    g.add_child(i)
for i in range(3):
    g.add_child(i)
for i in range(7, 9):
    g.add_child(i)
for i in range(3):
    g.add_child(12)  # add 3 children to node 12
g.add_edge(1, 16)
...
labs1 = pts.labels(v1, scale=0.02, italic=True).shift(0.05, 0.02, 0).c("green")
labs2 = pts.labels(v2, scale=0.02, precision=3).shift(0.05, -0.02, 0).c("red")
