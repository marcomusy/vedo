...
def load_data():
    ...
    return data, allconf, allreco, alldeat
...
for place, theta, phi, confd, deaths, recos in data:
    ...
    Sphere(pos, np.power(confd,  1/3)/750, alpha=0.4, res=12).flag(fl)
    Sphere(pos, np.power(deaths, 1/3)/750, alpha=0.4, c='k', res=10)
...
Text2D('COVID-19 spread on '+date
       ...
       +'\n#recovd: '+str(allreco))
Earth()
show(..., axes=12, bg2='lb', zoom=1.5, elevation=-70)
