...
class SubMesh:
    ...
    * dist2cut: distances from the original vertices in the submesh (old_pids) to the cut
    ...
    def glue_(self, radius, align):
        ...
        if align:
            sm.align_with_landmarks(
                self.submesh.vertices[self.new_pids], self.cut.vertices, rigid=True
            )
        if radius > 0:
            if (
                len(self.dist2cut) == 0
            ):  # pre-compute the distances for interactive gluing
            ...
            for i in self.old_pids:
                d = min(self.dist2cut[i] / radius, 1.0)
                self.mesh.vertices[self.submesh.pointdata["pids"][i]] = (
                    d * sm.vertices[i]
                    + (1 - d)
                    * self.original_mesh.vertices[self.submesh.pointdata["pids"][i]]
                )
        else:
            for i in self.old_pids:
