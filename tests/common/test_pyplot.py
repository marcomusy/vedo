...
def test_pyplot_figure_composition() -> None:
    ...
    fig += Mesh(shapes.Cylinder().dataset).scale(0.7).pos(7, 4).rotate_x(-90, around=[7, 4, 0])
    fig += Image(np.full((16, 16, 3), 180, dtype=np.uint8)).scale(0.005).pos(2, 10)
    fig += Points([[8, 1], [10, 3]], r=15)
    fig += pie_chart([0.1, 0.2, 0.3, 0.1, 0.3], c=[1, 2, 3, 4, "w"]).scale(1).pos(1, 6, 0.2)
    ...
    fig += shapes.Arrow([2, 3], [4, 5]).z(0.05)
    fig += shapes.Line([2, 3], [4, 5]).z(0.1).lw(2)
    fig += shapes.Line([2, 2], [4, 4], c="k", lw=6).z(0.1)

    fig += shapes.DashedLine([8, 3], [10, 5], spacing=0.5, c="r")
    fig += shapes.Tube([[8, 2, 0], [10, 4, 0]], r=0.1).lighting("ambient")
    ...
    fig += shapes.Brace([2, 1], [4, 3], comment="Brace", padding1=0, italic=3).z(0.1)
    fig += shapes.Ribbon(
        shapes.Spline([[4, 0], [5, 2], [6, 0], [7, 0.5]]),
    ...
    fig += shapes.Cone().scale(2).pos(10, 6).rotate_y(90, around=[10, 6, 0])
    fig += (
        shapes.Text3D("MyTest3D", c="k", justify="center", font="Quikhand")
        .pos(5, 11)
        .scale(0.5)
        .rotate_z(20, around=[5, 11, 0])
    )
