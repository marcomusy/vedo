...
class _BlenderStyleDragInfo:
    ...
    delta: np.ndarray = field(default_factory=lambda: np.array((0, 0, 0), dtype=float))
...
class BlenderStyle(vtki.vtkInteractorStyleUser):
    ...
    def perform_picking_on_selection(self):
        ...
        assemblyPath = renderer.PickProp(x0, y0, x1, y1)

        # re-pick in larger area if nothing is returned
        if not assemblyPath:
            assemblyPath = renderer.PickProp(x0 - 2, y0 - 2, x1 + 2, y1 + 2)

        # The nearest prop (by Z-value)
        if assemblyPath:
            nearest_node = assemblyPath.GetItemAsObject(0)
            if not nearest_node:
                return []
            nearest_prop = nearest_node.GetViewProp()
